...
public class BwPortService : IBwPortService {
    ...
    public async Task NotifyContentScriptsOfRestartAsync() {
        _logger.LogInformation("Notifying content scripts of service worker restart");

        try {
            var tabs = await _webExtensionsApi.Tabs.Query(new WebExtensions.Net.Tabs.QueryInfo());
            var notifiedCount = 0;

            foreach (var tab in tabs) {
                if (tab.Id.HasValue) {
                    try {
                        await _webExtensionsApi.Tabs.SendMessage(tab.Id.Value, new { type = "SW_RESTARTED" });
                        notifiedCount++;
                    }
                    catch {
            ...
            _logger.LogInformation("Notified {Count} tabs of service worker restart", notifiedCount);
        }
        catch (Exception ex) {
            _logger.LogError(ex, "Error notifying content scripts of restart");
        }
    }
