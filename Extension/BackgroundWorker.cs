...
public partial class BackgroundWorker : BackgroundWorkerBase, IDisposable {
    ...
    // HandleAppRequestIpexGrantRpcAsync removed — replaced by HandleAppRequestSubmitIpexGrantRpcAsync
    // (decoupled issuance from submission so the App dialog can review the credential between steps)

    // HandleGrantFromApplyAsync removed — replaced by HandleAppRequestIssueEcrCredentialRpcAsync
    // (issuance) + SubmitGrantPostIssueAsync (grant with pre-issued acdc/anc/iss)
    // HandleGrantFromAgreeAsync removed — replaced by SubmitGrantFromAgreeAsync
