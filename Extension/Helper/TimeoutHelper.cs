...
public static class TimeoutHelper {
    ...
    */
    ...
    /*
    * Wraps an asynchronous ValueTask operation with a timeout. If the operation does not complete within the specified timeout, it returns a failure result.
    * Otherwise, it returns the result of the operation.
    */
    public static async Task<Result<T>> WithTimeout<T>(Func<CancellationToken, ValueTask<T>> operation, TimeSpan timeout) {
        // Create separate cancellation tokens for timeout and operation
        using var timeoutCts = new CancellationTokenSource(timeout);
        using var operationCts = new CancellationTokenSource();
