...
function getTabUrlAndContinue(tabId) {
    try {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    // || typeof message.iframeMode !== 'boolean'
        ...
        let messageOrigin = sender.origin;
        // tmp
        ...
        switch (message.name) {
            case "getTabUrlResponse":
                ...
                if (typeof message.sourceHostname !== 'string') {
                    ...
                    chrome.permissions.contains({ origins: [origin] }, (isOriginPermitted) => {
...
async function RegisterContentScripts() {
    ...
    try {
        await chrome.scripting.unregisterContentScripts(async function () {
            try {
