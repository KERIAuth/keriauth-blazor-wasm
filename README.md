...
## Overview
**DIGN** is an identity wallet packaged as a Chromium browser extension, designed to accelerate adoption of establishing secure and authentic trust between individuals and websites they visit. It is based on standards for cryptographic and legal roots of trust: decentralized key management and identifiers (KERI), verifiable credentials (ACDC), and GLEIF's Verifiable Legal Entity Identifier (vLEI) ecosystem.
...
* **Connection to a KERIA Service** — [KERIA](https://github.com/weboftrust/keria) is a multi-tenant service that provides infrastructure for Signify clients. KERIA creates a separate agent instance for each client and partitions storage to isolate agents. The agent does not hold the controller's signing keys; signing is performed by the Signify client (DIGN). The agent manages KERI event logs, credential (ACDC) storage, and message exchange with other agents on behalf of the controller.
