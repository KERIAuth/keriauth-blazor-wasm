...
export async function isServiceHealthy(healthCheckUrl: string): Promise<boolean> {
  ...
  try {
    ...
    console.error(`Failed to connect to service. It may be down. Error:`, (error as Error).message);
    return false;
  }
}

// Default KERIA connection parameters (adjust as needed for your environment)
// Note: Use localhost when accessing from browser extension (not from within Docker)
...
export async function initializeAndConnectClient(
    ...
    try {
        ...
        await client.connect(); // TODO P3 is connect needed after a boot?
 ...
 */
export async function createNewAID<T>(
    ...
    try {
        ...
        return { aid: newAidInceptionEvent, operation: completedOperation as Operation<T> };
    } catch (error) {
 ...
 */
export async function addEndRoleForAID<T>(
    client: SignifyClient,
    aidAlias: string,
    role: string
    ...
    try {
        ...
        return { operation: completedOperation as Operation<T> };
    } catch (error) {
 ...
 */
export async function resolveOOBI<T>(
    client: SignifyClient,
    oobiUrl: string,
    contactAlias?: string
    ...
    try {
        ...
        return { operation: completedOperation as Operation<T>, contacts: contact };
    } catch (error) {
 ...
 */
export async function verifyChallengeResponse<T>(
    client: SignifyClient,
    ...
    try {
        ...
        if (completedOperation.error) {
            ...
            await client.operations().delete(completedOperation.name);
            return { verified: false, operation: completedOperation as Operation<T> };
        }
        ...
        return { verified: true, said: said, operation: completedOperation as Operation<T> };
    } catch (error) {
