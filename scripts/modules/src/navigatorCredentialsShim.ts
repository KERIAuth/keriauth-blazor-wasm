...
function extractAaguidFromAuthenticatorData(authData: ArrayBuffer): string {
    ...
    console.debug(`[navigatorCredentialsShim] extractAaguid - authData length: ${bytes.length} bytes`);

    if (bytes.length < 37) {
        console.debug(`[navigatorCredentialsShim] extractAaguid - authData too short for flags check`);
    ...
    console.debug(`[navigatorCredentialsShim] extractAaguid - flags: 0x${flags.toString(16)}, ` +
        `AT=${hasAttestedCredentialData}, ED=${hasExtensions}`);

    if (!hasAttestedCredentialData) {
        console.debug(`[navigatorCredentialsShim] extractAaguid - AT flag not set, no attested credential data`);
        return '00000000-0000-0000-0000-000000000000';
    }

    if (bytes.length < 55) {
        console.debug(`[navigatorCredentialsShim] extractAaguid - authData too short for AAGUID (need 55, have ${bytes.length})`);
        return '00000000-0000-0000-0000-000000000000';
    }
    ...
    console.debug(`[navigatorCredentialsShim] extractAaguid - extracted AAGUID: ${aaguid}`);
...
export async function createCredential(
    ...
    if (typeof response.getAuthenticatorData === 'function') {
        ...
        console.debug(`[navigatorCredentialsShim] createCredential - using getAuthenticatorData()`);
        ...
        console.debug(`[navigatorCredentialsShim] createCredential - getAuthenticatorData() not available, trying attestationObject`);
        try {
    ...
    console.debug(`[navigatorCredentialsShim] createCredential - AAGUID: ${aaguid}`);
