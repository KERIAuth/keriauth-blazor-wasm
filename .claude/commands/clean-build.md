...
1. Restore NuGet dependencies:
   - Run `dotnet restore` to ensure NuGet packages are available
...
4. Build TypeScript FIRST (critical for BackgroundWorker.js generation):
   - Run `npm run build` in the Extension directory
   - Verify compilation succeeds before proceeding

5. Build the C# project with quick mode (TypeScript already built):
   - Run `dotnet build -p:Quick=true` from project root
   - This skips redundant TypeScript compilation since step 4 already built it
